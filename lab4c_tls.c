...
void handle_shutdown(int fd){
	...
	char sd_buff[30];
	sprintf(sd_buff, "%s SHUTDOWN\n", sd_time_string);
	if ((SSL_write(ssl, sd_buff, strlen(sd_buff))) < 0){
		fprintf(stderr, "Error: unable to write shutdown report to TLS session. %s\n", strerror(errno));
		exit(2);
	}
	if (log_arg){
...
void process_invalid_command(){
	char iv_comm_buff[30];
	sprintf(iv_comm_buff, "%s\n", "Error: invalid command.");
	if ((SSL_write(ssl, iv_comm_buff, strlen(iv_comm_buff))) < 0){
		fprintf(stderr, "Error: unable to write invalid command report to TLS session. %s\n", strerror(errno));
		exit(2);
	}
...
int main (int argc, char* argv[]){
	...
	SSL_library_init();
	//register error strings for all libcrypto functions
	SSL_load_error_strings();

	//add all algorithms to table
	...
	if (context == NULL){
		fprintf(stderr, "Error: unable to create new SSL_CTX object. %s\n", strerror(errno));
		exit(2);
	}
	ssl = SSL_new(context);				//create new SSL structure to hold data for TLS connection
	if (ssl == NULL){
		fprintf(stderr, "Error: unable to create new SSL structure. %s\n", strerror(errno));
		exit(2);
	}
	//set file descriptor as input/output facility for TLS
	if ((SSL_set_fd(ssl, sock_fd)) == 0){
		fprintf(stderr, "Error: unable to set file descriptor. %s\n", strerror(errno));
		exit(2);
	}
	//initiate TLS handshake with a server
	if ((SSL_connect(ssl)) != 1){
		fprintf(stderr, "Error: unable to build TLS session. %s\n", strerror(errno));
		exit(2);
	}

	//send and log ID
	char id_buff[20];										//buffer that holds ID string
	sprintf(id_buff, "ID=%s\n", id_num);
	if ((SSL_write(ssl, id_buff, strlen(id_buff))) < 0){
		fprintf(stderr, "Error: unable to write ID to TLS session. %s\n", strerror(errno));
		exit(2);
	}
	if (log_arg){
		dprintf(log_fd, "ID=%s\n", id_num);
	...
	while (1){
		...
		//create report for each sample
		char report_buff[20];
		sprintf(report_buff, "%s %.1f\n", time_string, converted_temp);
		if ((SSL_write(ssl, report_buff, strlen(report_buff))) < 0){
			fprintf(stderr, "Error: unable to write report to TLS session. %s\n", strerror(errno));
			exit(2);
		}
		if (log_arg){
		...
		while (time_passed < period_duration){
			...
			if (pollfd_array[0].revents & POLLIN){
	            ...
	            int bytesRead = SSL_read(ssl, buff, 256);
	            if (bytesRead < 0){
	                    fprintf(stderr, "Error reading from TLS connection. %s.\n", strerror(errno));
	                    exit(1);
	            }
	...
	//shut down SSL session
	if (SSL_shutdown(ssl) == -1){
		fprintf(stderr, "Error: unable to shut down SSL session. %s\n", strerror(errno));
		exit(2);
	}
